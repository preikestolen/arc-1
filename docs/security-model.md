...
| R6 | **Principal-propagation fallback identity.** Default PP mode now fails closed on JWT propagation failure. Operators can still explicitly set `SAP_PP_STRICT=false`, which falls back to the shared client on PP failure and must be treated as an accepted break-glass/migration posture, not a production default. | Med when explicitly disabled | PP only | [`config.ts`](../src/server/config.ts), [`server.ts`](../src/server/server.ts) | mitigated/default-closed by [#488](https://github.com/arc-mcp/arc-1/pull/488); explicit non-strict fallback remains opt-in risk |
...
| R15 | **Cleartext SAP source at rest.** SQLite disk cache stores full SAP source unencrypted. Cache DB and file audit sink now use private `0600` mode, but plaintext source remains visible to a local actor who can read the service account's files or backups. Use encrypted volumes or `ARC1_CACHE=none` for IP-sensitive landscapes. | Low/Med | disk cache | [`sqlite.ts`](../src/cache/sqlite.ts), [`file.ts`](../src/server/sinks/file.ts) | partial: [#496](https://github.com/arc-mcp/arc-1/pull/496) fixed file perms; plaintext source-at-rest remains an accepted/operator-controlled risk |
...
| R17 | **Hardening cluster (low):** API-key docs/compare not constant-time ([`http.ts`](../src/server/http.ts), inconsistent with the HMAC paths); `change_package` compiles an unbounded `objectType` into a RegExp (bounded impact). The PR-title shell interpolation and ACTION_POLICY completeness-guard subitems are now closed. | Low | mixed | various | partial: PR-title fixed in [#550](https://github.com/arc-mcp/arc-1/pull/550); ACTION_POLICY guard exists in `npm run validate:policy`; API-key timing + `change_package` regex remain open |
