...
- `docs/research/2026-04-10-test-reliability-audit-points-2-7.md`
- `docs/plans/completed/2026-04-10-test-1-reliability-telemetry.md`
- `docs/plans/completed/2026-04-10-test-2-skip-policy-pseudo-skips.md`
- `docs/plans/completed/2026-04-10-test-3-coverage-ci-signal.md`
- `docs/plans/completed/2026-04-10-test-4-trycatch-signal-quality.md`
- `docs/plans/completed/2026-04-10-test-5-crud-lifecycle.md`
- `docs/plans/completed/2026-04-10-test-6-btp-stability.md`
- `docs/plans/completed/2026-04-10-test-7-docs-verification.md`
- `docs/plans/completed/2026-04-10-test-reliability-audit-points-2-7-hardening.md`
...
- If read-only profiles are parallelized, run each shard with its own MCP server and explicit `ARC1_MAX_CONCURRENT`. Start with two shards and `ARC1_MAX_CONCURRENT=5` each, then docs/compare server logs for timeout, 5xx, reset, and enqueue categories before increasing.
...
- `tests/fixtures/abap/zi_arc1_i33_root.ddls.abap:2` contains `@AbapCatalog.compiler.docs/compareFilter: true`, which this S/4 system rejects for view entities.
- `ZI_ARC1_I33_ROOT` failed activation with "Annotation `AbapCatalog.compiler.docs/compareFilter` is not allowed in view entities".
...
- The invalid fixture source makes this visible: `tests/fixtures/abap/zi_arc1_i33_root.ddls.abap` uses classic CDS annotations on a view entity. On this S/4 system, `@AbapCatalog.compiler.docs/compareFilter` is rejected for view entities. The dependent projection then fails because the root is inactive.
...
- Fix the fixture first: remove `@AbapCatalog.sqlViewName` and `@AbapCatalog.compiler.docs/compareFilter` from the view-entity fixture unless a release-specific reason requires them. Keep only annotations valid for view entities on the supported S/4 target.
...
- Items 8-13 are implemented by `docs/plans/completed/2026-06-06-test-runtime-profiles-and-coverage.md`.
