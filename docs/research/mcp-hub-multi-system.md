...
| L4 | **`jwt-bearer` is NOT enabled by default** — the grant-types allowlist `[authorization_code, refresh_token]` blocks it; the issuer check merely runs *first* | Live (corrected 2026-06-17): a **same-subaccount user token** (trusted issuer → issuer check passes) → `invalid_client / "Unauthorized grant type"`, identical to `client_credentials`. The earlier CF-token `"Unable to map issuer"` was the issuer check failing *before* the grant-type check, **masking** this. **Rule: the grant type is checked against whichever OAuth client authenticates the `POST /oauth/token` request.** For *this* hub topology that is the HUB's client (hub creds + `granted-apps` chain), so the grant goes in the **hub's** xsuaa and ARC-1 only needs `granted-apps` + audience-validate. It is **not** a general rule — a consumer that exchanges with a service key of **ARC-1's own** xsuaa needs the grant in **ARC-1's** descriptor instead (added 2026-07-22, [PR #605](https://github.com/arc-mcp/arc-1/pull/605); see [issues/605-xsuaa-jwt-bearer-grant.md](issues/605-xsuaa-jwt-bearer-grant.md)). That route needs no grant chain at all and is the simpler one for a single consumer. |
