...
- Docs updated: `CLAUDE.md` Key Files row + `docs/tools.md` warnings field documentation +
  `docs/roadmap.md` entry marked as shipped + `docs/compare/00-feature-matrix.md` if a row fits.
...
| `docs/compare/00-feature-matrix.md` | Add or update a row for "ARC-1-native pre-write semantic hints" if the matrix has a relevant column; otherwise skip. |
...
- Modify: `docs/compare/00-feature-matrix.md` (only if a relevant row fits)
...
- [ ] In `docs/compare/00-feature-matrix.md`, scan for a row covering "pre-write semantic
...
- [ ] Inspect the diff: `git diff main -- src/lint/ tests/unit/lint/ CLAUDE.md docs/tools.md docs/roadmap.md docs/compare/00-feature-matrix.md` — only the planned files changed; no incidental edits.
