...
ADT evidence from Eclipse 3.58 and the docs/compare docs confirms the route shape: class main source is `/sap/bc/adt/oo/classes/{name}/source/main`, while local sections are `/sap/bc/adt/oo/classes/{name}/includes/{definitions|implementations|macros|testclasses}`. Existing ARC-1 CRUD code already supports lock parent object plus PUT arbitrary source URL with the lock handle; `scaffold_rap_handlers` already uses one parent class lock for multi-include saves, which matches the include-lock analysis in `docs/compare/abap-adt-api/evaluations/issue-36-include-lock.md`.
...
| `docs/compare/00-feature-matrix.md` | Competitor matrix row for RAP behavior-pool scaffolding |
| `docs/compare/adt/apis/03-oo-classes-and-interfaces.md` | ADT API evidence for class include routes |
...
- Modify: `docs/compare/adt/apis/03-oo-classes-and-interfaces.md`
- Modify: `CLAUDE.md`
...
- [x] Update `docs/compare/00-feature-matrix.md` row for RAP behavior-pool handler scaffolding from in-flight/partial to supported with CLAS include writes and auto-skeletons.
- [x] Update `docs/compare/adt/apis/03-oo-classes-and-interfaces.md` to record class include PUT routing with parent class locking.
...
- Move: `docs/plans/completed/2026-05-10-pr-a-clas-include-rap-skeletons.md` to `docs/plans/completed/2026-05-10-pr-a-clas-include-rap-skeletons.md`
